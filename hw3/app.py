...
bank = Image.open('hw3/pictures/bank.png')

_, _, _, _, scaler = preprocess_data(open_data('hw3/data/df.csv'))
...
def render_page(bank):
    ...
    with tab1:
        ...
        description_cat = pd.read_csv('hw3/data/description_cat.csv', index_col='Unnamed: 0')
        st.dataframe(description_cat)

        st.subheader('Описательные статистики: вещественные переменные')
        description_num = pd.read_csv('hw3/data/description_num.csv', index_col='Unnamed: 0')
    ...
    with tab2:
        ...
        with col3:
            ...
            work_time = st.slider('Укажите сколько клиент уже работает на текущем месте в месяцах', min_value=1, max_value=500)
        st.divider()
        ...
        with col2:
            first_payment = st.slider('Сколько составил первоначальный взнос по кредиту', min_value=1, max_value=140000)
    ...
    with tab3:
        feature_importances = Image.open('hw3/pictures/feature_importances.png')
        st.write('**Важные для модели признаки:**')
        st.image(feature_importances)
        
        shap = Image.open('hw3/pictures/shap.png')
        st.write('**SHAP**')
        st.image(shap)
